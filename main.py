...
def main(args):
    ...
    trunk_optimizer = torch.optim.Adam(trunk.parameters(), lr=args.lr_const, weight_decay=args.wd)
    #embedder_optimizer = torch.optim.Adam(embedder.parameters(), lr=lr, weight_decay=weight_decay)
    classifier_optimizer = torch.optim.Adam(classifier.parameters(), lr=args.lr_const, weight_decay=args.wd)
    ...
    # Create the tester
    tester = testers.GlobalEmbeddingSpaceTester(normalize_embeddings=True, batch_size=args.batch_size, dataloader_num_workers=args.num_data_workers,
